         ...
         <header className="w-full py-4 flex justify-between items-center flex-shrink-0">
          <div className="flex items-center gap-2 sm:gap-3">
            <h1 className="text-xl sm:text-2xl font-bold flex items-center">
                <span className="w-8 h-8 sm:w-10 sm:h-10 bg-green-500 rounded-full flex items-center justify-center text-slate-900 font-extrabold text-lg">
            ...
            <div className="flex flex-col items-start">
              <span className="text-xs sm:text-sm text-slate-400">أهلاً بك يا بطل</span>
              <span className="font-bold text-base sm:text-lg text-green-400 -mt-1 truncate max-w-[100px] sm:max-w-none">{studentName}</span>
            </div>
          </div>

          <div className="flex items-center gap-2 sm:gap-3">
            <div className="flex items-center gap-1 sm:gap-2 bg-slate-800 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full border border-slate-700">
                <span className="text-lg sm:text-xl">🔥</span>
                <span className="font-bold text-base sm:text-lg text-white">{streak}</span>
            </div>
            <div className="flex items-center gap-1 sm:gap-2 bg-slate-800 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full border border-slate-700">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 sm:h-6 sm:w-6 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <span className="font-bold text-base sm:text-lg text-white">{points}</span>
            </div>
             <button onClick={handleChangeUser} className="bg-slate-700 hover:bg-slate-600 p-2 sm:p-2.5 rounded-full" aria-label="تغيير المستخدم">
