...
const EducationalCards: React.FC<EducationalCardsProps> = ({ onBack }) => {
  ...
  const fetchCard = useCallback(async () => {
    ...
    try {
      ...
      setError("عذراً، حدث خطأ أثناء تحضير البطاقة. يرجى المحاولة مرة أخرى.");
    } finally {
      setLoading(false);
    }
  }, [cardCount]);
  ...
  return (
    <ExerciseContainer title="بطاقات تعليمية" onBack={onBack} exerciseNumber={cardCount}>
      ...
      {error && (
          <div className="text-center flex flex-col items-center justify-center flex-grow gap-4">
              <p className="text-red-400 text-lg">{error}</p>
              <button onClick={fetchCard} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-lg transition-colors">
                  حاول مرة أخرى
              </button>
          </div>
      )}
